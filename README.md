[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/9DrT6EZJ)
# Potatoland

A native potato dish sharing/recommendations app.

## Screenshots
![Start screen](assets/start.png)
![Upload overlay](assets/upload-overlay.png)
![List item - French fries](assets/fries.png)

## What It Does
Ideally, this app would be like a social media platform where users can share their own potato dishes and leave ratings and comments on other uses' recommendations. For the scope of this project, I created the app as if it were viewed from one user's perspective. In this app, users are able to browse through a collection of potato dishes, leave ratings and comments, as well as share their own favorite potato dishes.
